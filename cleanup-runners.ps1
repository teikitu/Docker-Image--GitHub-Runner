...
$runnerBaseName = "Node-"
$runnerListJson = gh api -H "Accept: application/vnd.github.v3+json" "/orgs/$org/actions/runners"
...
Foreach ($runner in $runnerList) {
    try {
        If (($runner.name -like "$runnerBaseName*") -and ($runner.status -eq "offline")) {
            ...
            gh api --method DELETE -H "Accept: application/vnd.github.v3+json" "/orgs/$org/actions/runners/$($runner.id)"
        }
    }
    catch {
